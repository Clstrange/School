...
function newGame() {
	...
	for(button of document.querySelectorAll("div button")){
		button.innerHTML = "";
		button.class = null;
...
function makeComputerMove() {
	let test = document.querySelectorAll("div button");
	let number;

	do {
		number = Math.floor(Math.random() * test.length);
		console.log("loading")
	} while(test[number].disabled);

	test[number].innerHTML = "O";
	test[number].classList.add("o");
	test[number].disabled = true;
	switchTurn();
}