...
function showCard(index) {
   ...
   $cardDivs[index].slideUp(200).queue(function(next){
...
function showMatch(cardIndex1, cardIndex2) {
   ...
   setTimeout(function(){
      $($cardDivs[cardIndex1]).animate({fontSize: "+=25"},200)
      $($cardDivs[cardIndex2]).animate({fontSize: "+=25"},200)
      $($cardDivs[cardIndex1]).animate({fontSize: "-=25"},200)
      $($cardDivs[cardIndex2]).animate({fontSize: "-=25"},200)
   },500)
...
function newGame() {
   // TODO: Animate fading in and out the cards
      ...
      $("#cardGrid").fadeOut().queue(function(next){
         // Hide all cards 
